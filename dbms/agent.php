...
$result=$conn->query($sql);
			if($result->num_rows>0)
            {
              ...
              {
              	$a=$row["rid"];
              	if(isset($_POST["$a"]) && isset($_POST["client"]))
              	{
              		$b=$_POST["client"];
              		$c=$_SESSION["id"];
              		$d=$row["cost"];
				      ...
				      $wor=implode(",", $wo);
              		$l="Insert into log(rid,aid,pan,wid,cost) values('$a','$c','$b','$wor','$d')";
...
<form method="post">
	<font color="black">
		<center>
			<h2>Select the client:
	<select id="types" name="client">
		<option value="">Select Client</option>
		<?php
		$a=$_SESSION["id"];
			$sql="Select * from agent_client where aid='$a'";
			$result=$conn->query($sql);
			if($result->num_rows>0)
            {
              while($row=$result->fetch_assoc())
              {
		?>
		<option value="<?php echo $row["pan"];?>"><?php echo $row["pan"];?></option>
          <?php
      }}
      ?>
    </select>
...
{
	$type=$_POST["ptype"];
	$sql="Select * from large where type='$type'";
}
if(isset($_POST["rsub"]))
{
	$sql="Select * from large";
